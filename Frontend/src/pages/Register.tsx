...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await authAPI.register({ username, email, password, purpose: purpose || undefined, role, manager_code: role === 'worker' ? managerCode || undefined : undefined } as any);
    ...
    <div style={s.page}>
      ...
      <div style={s.card}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {role === 'worker' && (
            <div style={s.field}>
              <label style={s.label}>Manager Code</label>
              {managers.length > 0 ? (
                <select style={{ ...s.input, cursor:'pointer' }} value={managerCode} onChange={e => setManagerCode(e.target.value)}>
                ...
                <input style={s.input} className="fi" type="text" placeholder="e.g. MGR-GEEVIN-4821"
                  value={managerCode} onChange={e => setManagerCode(e.target.value.toUpperCase())} />
              )}
              <p style={{ fontSize:11, color:'#94a3b8', marginTop:4 }}>Ask your manager for their unique code</p>
            </div>
          )}
